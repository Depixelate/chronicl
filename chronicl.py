...
try:
    ...
    print(
        "Can't read the arguments. The date format should be YYYY-MM-DD, e.g. 2014-08-15\n"
    )
    quit()
try:
...
if len(workspace_ids_names) > 1:
    ...
    try:
        wnum = int(input("Enter 0,1,2 etc.: "))
...
clients_response = requests.get(url, headers=headers, params=params)
        ...
        client_project_list.append(
            {
                ...
                "weekly_hours": [],
                "color": colordict[int(project["color"])],
            }
        )
...
for item in client_project_list:
    print(
        str(client_project_list.index(item))
        + " : "
        + item["client_name"]
        + " : "
        + item["project_name"]
...
if clients_or_projects == "p":
    ...
    if positions != "a":
        ...
        client_project_list = [
            item
            for item in client_project_list
            if client_project_list.index(item) in positions
        ]
    ...
    if positions != "a":
        ...
        try:
            ...
            quit()
        clientnames = [
            name for name in clientnames if clientnames.index(name) in positions
        ]
